...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = 'Welcome to MAC Management System'
...
def profile(request):
    use = request.user
    logged_user = UserProfile.objects.filter(user=use).get()
    if request.method == 'POST':
        ...
        if img_form.is_valid():
            ...
            updated = True
            context = {'img_form' : img_form, 'updated' : updated , 'logged_user': logged_user}
